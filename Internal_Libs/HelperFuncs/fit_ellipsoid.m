function sol = fit_ellipsoid( data )
    %
    ...
    %
    % Inputs:
    % * data      - Cartesian data, 3xN matrix
    %
    % Output:
    ...
    % September, 2015
    %
    % Simplified by Gabriel Antoniak (gjantoni@umich.edu) to remove constraint
    % cases to make code more readable. Our use case for this is to fit to an
    % ellipsoid to magnetometer data, which will have tens if not hundreds or
    % thosands of points.
    
    
    if size( data, 1 ) ~= 3
        error( 'Input data must have three rows!' );
    else
        x = data( 1, :)'; % convert to columns for linear algebra
        y = data( 2, :)';
        z = data( 3, :)';
    end
