...
classdef Tag
    ...
    methods
        ...
        function self = trial_extraction(self)
            ...
            if ~isempty(self.temp_imu)
                self.temp_imu = self.temp_imu(s:e);
            end

            if ~isempty(self.temp_pres)
                self.temp_pres = self.temp_pres(s:e);
            end
        ...
        function self = plot_temperatures(self)
            fprintf("Plotting temperatures for " + self.name + "\n");
            if isempty(self.temp_imu) && isempty(self.temp_pres)
                fprintf("\tNo temperature data for " + self.name + "\n");
            end

            num_temp_streams = 0;
            if ~isempty(self.temp_imu)
                num_temp_streams = num_temp_streams + 1;
            end
            if ~isempty(self.temp_pres)
                num_temp_streams = num_temp_streams + 1;
            end
            names(1:num_temp_streams) = {0};
            for i = 1:num_temp_streams
                names{i} = "";
            end
            n = 1;
            ...
            if ~isempty(self.temp_imu) 
                ...
                names{n} = "IMU Temperature";
                n = n + 1;
            end
            if ~isempty(self.temp_pres)
                plot(self.time, self.temp_pres, 'b-');
                names{n} = "Pressure Temerature";
                n = n + 1;
            end
