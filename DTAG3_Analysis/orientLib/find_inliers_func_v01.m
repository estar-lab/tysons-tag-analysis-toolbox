function [inliers_out, fad_status] = find_inliers_func_v01(Acc_1, Depth_1, Acc_2,...
...
%
% In this version:
% For each dataset, we calculate the median distance from each point to its 
% K near neighbors. Then subsample the points using the distance as weights. 
% So dense points become sparse after subsample. In this way, the resulting 
% points distribution hold the same 3D shape and also have samlilar point 
% densities. 
%
%
%
...
% OUTPUT:
% inliers_out   n2-by-1 binary values indicate whether or not a point in
...
% For each set, calculate the median distance from each point to its K
% neighbors. Subsample the points using the distance as weights. So dense
% points become sparse after subsample. In this way, the resulting points
% distribution hold the same 3D shape and have samlilar point densities.
K = 30;
sf_down = 5;
for i_set = 1:2  
  n_total{i_set} = length(A{i_set});
  % Use down sampled set as reference for finding k nearest neighbors.
  i_ref = 1:ceil(sample_freq/sf_down):n_total{i_set};

  [dmvar01, dist_mat] = knnsearch(A{i_set}(i_ref,:), A{i_set},...
    'K', K, 'Distance', 'euclidean');
  dist = mean(dist_mat, 2);
  weights = dist.^2;
  
  % Sample k_use that has the same size as reference.
  k_use = round(sf_down/sample_freq*n_total{i_set});
  
  i_use{i_set} = sort(randsample(n_total{i_set}, k_use, true, weights));
  
  A{i_set} = A{i_set}(i_use{i_set},:);
  Depth_speed{i_set} = Depth_speed{i_set}(i_use{i_set},:);
end

if 0
  %% Testing mapping from dists to weights.
  Acc = movmean(Acc_1, ceil(0.1*sample_freq));
  n = length(Acc);
  i_ref = 1:ceil(sample_freq/sf_down):n;

  [dmvar02, dist_mat] = knnsearch(Acc(i_ref,:), Acc,...
    'K', K, 'Distance', 'euclidean');
  dist = mean(dist_mat, 2);
  w = dist.^2;
  
  k_use = round(sf_down/sample_freq*n);
  
  idx = sort(randsample(n, k_use, true, w));
  
  [dmvar03, dist_mat_2] = knnsearch(Acc(i_ref,:), Acc(idx, :),...
    'K', K, 'Distance', 'euclidean');
  dist_2 = mean(dist_mat_2, 2);
...
% Prepare output.
% [dmvar05, i_unique, dmvar06] = unique(i_use{2});
% inliers_out = logical(...
%   interp1(i_use{2}(i_unique), double(inliers(i_unique)), 1:n_total{2},...
%   'nearest', 'extrap'));
inliers_out = nan(n_total{2}, 1);
inliers_out(i_use{2}) = inliers;
