...
%
%     Edit by Joaquin Gabaldon 21-03-12: convert inclination to front-right-down
%           convention and correct commenting
...
cp = cos(-p) ;      % Negate pitch to force match f-r-d right-hand convention
sp = sin(-p) ;
cr = cos(r) ;
sr = sin(r) ;
Tx = [cp -sr.*sp -cr.*sp] ;
...
% heading estimate in right-hand system (front-right-down)
h = atan2(-Mh(:,2),Mh(:,1)) ;

% compute mag field intensity and inclination
v = sqrt(Mh.^2*[1;1;1]) ;         % compute magnetic field intensity
incl = real(asin(Mh(:,3)./v)) ;  % compute inclination in f-r-d convention
