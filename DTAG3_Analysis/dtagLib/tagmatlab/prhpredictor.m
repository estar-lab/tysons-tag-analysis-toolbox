...
fprintf(['PRHPREDICTOR: Estimate orientation of tag w.r.t. animal\n', ...
    'Click near interval edge (black) to move edge to clicked ', ...
    'location. Only edge closest to the mouse will move.\n***Note***: ', ...
    'Lower PRH condition values are better.\n'])
fprintf(['METHOD 1: Long (surface) and short (ascent/descent) intervals\n', ...
    '\tResize long intervals to cover time where animal is ONLY at/close ', ...
    'to the surface.\n\tResize short intervals to cover time where animal ', ...
    'is ONLY performing ascent/descent.\n'])
fprintf(['METHOD 2: Long (surface) intervals only\n', ...
    '\tResize long intervals to cover time where animal is ONLY at/close ', ...
    'to the surface.\n'])
fprintf(['OTHER CONTROLS:\n\tx: Discard current interval/s, ', ...
    'proceed to next ascent/descent.\n\tq: Quit PRH predictor, no more ', ...
    'intervals will be processed.\n\tAny other button: Save and print ', ...
    'predicted PRH results from current interval.\n\t\tOnly saved results ', ...
    'will be returned by prhpredictor.\n'])
...
%             tdir,prh*180/pi,pow) ;
      ss = sprintf('cue=%6.0f %s: p0=%3.1f, r0=%3.1f, h0=%3.1f, cond=%4.3f, rms=%4.3f\n',...
...
subplot(313), plot(PRH(:,1),PRH(:,6),'*-'); grid, set(gca,'XLim',XL,'YLim',[0 0.5]) % orig 0.1
hold on, plot(PRH(end,1),PRH(end,6),'o'), hold off, xlabel('time cue'), ylabel('Condition')
figure(2)
