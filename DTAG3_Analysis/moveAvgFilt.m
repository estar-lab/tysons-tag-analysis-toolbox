...
for nn = 1:n
    ...
    % Nope, use local mean to replace nan instead.
    Xundelay(indNan) = mean(Xvec(indNan))*ones(size(Xvec(indNan)));
    ind_begin_sec = 1:round(windowLength/2);
    Xundelay(ind_begin_sec) = mean(Xvec(ind_begin_sec))*...
      ones(size(Xvec(ind_begin_sec)));
    Xout(:,nn) = Xundelay';
end