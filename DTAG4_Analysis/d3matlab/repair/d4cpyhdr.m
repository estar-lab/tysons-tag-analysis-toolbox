function		d4cpyhdr(file1,file2)

%  d4cpycfg(file1,file2)
%  Copy the dtg header from file2 to the start of file1. This is only
%  needed if a dtg file is so corrupted that the header is missing or
%  unreadable. The result is put into a new file called the same as file1 
%  but with _repair.dtg at the end of the name.
...
% read the d4 header and first block headers
hdr2=fread(f2,80,'uchar') ;

if strcmp(char(hdr2(1:8)'),'DTAG 4.0')==0,
	fprintf('Reference file is not a valid D4 file\n')
	fclose(f2)
	return
end	
