function     RES = d3audit(recdir,prefix,tag,tcue,RES)
%
...
switch prefix(1:2),
   case 'hp',      % for harbour porpoise use:
      FH = 70000 ;       
      TC = 0.5e-3 ;           % power averaging time constant in seconds
      NS = 5 ;             % only read 5s at a time
      SOUND_DF = 3 ;     % decimation factor for playing sound
...
if isempty(gx), continue, end
      ...
      switch button,
         ...
         fn = sprintf('%s_ex%d.wav',tag,floor(cc(1))) ;
         d3wavcopy(recdir,prefix,[floor(cc(1)) ceil(cc(2))],fn) ;
         fprintf(' done\n');
...
function	doclicks(fn)

[cl,x,fs] = sortclicks(fn) ;
if ~isempty(cl),
	save([fn(1:end-3) 'mat'],'cl','x','fs') ;
end
return
