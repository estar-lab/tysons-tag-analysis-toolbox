...
kt = find(strcmp(type,'tempr')) ;       % find the temperature channel in X
...
% check all required fields are in the CAL
if ~isfield(CAL.MAG,'XC')
   CAL.MAG.XC = zeros(3) ;
end
if ~isfield(CAL.MAG,'PC')
   CAL.MAG.PC.POLY = zeros(3,1) ;
end

% if data-driven cal is not requested, just apply existing cal constants
% and return
if isempty(test),
   [M,fs,Mz] = full_rate_cal(X,CAL,km,kb,kt) ;
   return
end
...
if fst/df<fs,
   t = interp2length(t,fst/df,fs,size(s,1)) ;
end
...
% exclude magnetometer measurements with high change rate or that are too shallow
jj = [norm2(diff(M));0] ;
thr = prctile(jj,50) ;
kg = find((jj(ks)<thr) & (p(ks)>mindepth) & (any(isnan(M(ks,:)),2)==0)) ;
kk = ks(kg) ;
M = M(kk,:) ;

if isempty(M),
   fprintf(' No useable data deeper than %3.1f metres\n', mindepth) ;
   return
end
...
function    [M,fsm,Mz] = full_rate_cal(X,CAL,km,kb,kt)
%
...
if fst<fs,
   t = interp2length(t,fst,fs,size(s,1)) ;
end
[p,CAL,fsp] = d3calpressure(X,CAL,'none') ; 
if fsp<fs,
...
M = apply_cal(s,CAL,p) ;          % implement initial calibration
...
for k=1:3,
   ...
   M = apply_cal(s,CAL,p) ;      % implement improved calibration
end
...
M = apply_cal(s,CAL,p) ;      % implement original calibration
