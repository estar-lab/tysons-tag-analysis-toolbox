...
%
%		This generates a table which is an exact match to the table in crc.c in the D4 app code.
%
...
G(1) = hex2dec('1021') ;
for k=2:8,
   %G(k) = bitshift(uint16(G(k-1)),1) ;	% either way gives the same result
   G(k) = bitshift(G(k-1),1,'uint16') ;
   if bitget(G(k-1),16)
