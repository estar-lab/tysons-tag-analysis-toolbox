...
%
%     [X,fields] = readcsv(fname,comment,hdr)
...
%     character passed in comment (e.g., '%').
...
%		The field separation character is inferred from the header line of the file.
%		If hdr=-1, the field separation character must be a comma.
%
%     markjohnson@bio.au.dk
%        28 oct 2022: fixed bug with empty field names
%			4 nov 2022: fixed bug with FSEP when there is no header
%			9 april 2023: improved handling of lines with too few or many fields
...
if hdr>=0,
   while 1,
      ...
      if hh<0, break, end
      if isempty(FSEP),
         FSEP = which_field_separator(hh) ;
      end
      headers = parseline(hh,comment,FSEP) ;     % break header into field names
...
while 1,
   ...
   x = parseline(ss,comment,FSEP) ;
   if isempty(S) & nfields==0,
      nfields = length(x) ;
   end

   if length(x)>nfields,
      fprintf(' Warning: Too many columns on line %d of file %s\n',lk,fname) ;
		x = {x{1:nfields}} ;
   elseif ~isempty(x) & length(x)<nfields,
      fprintf(' Warning: Too few columns on line %d of file %s\n',lk,fname) ;
		x{length(x)+1:nfields} = [] ;
   end
...
function    x = parseline(s,comment,FSEP)
%
%
%
...
function     FSEP = which_field_separator(hh)
%
