function     RES = d3audit1(varargin)
%
      ...
      switch button,
         ...
         case 'p'
            ...
            if ~isempty(bs),
               ...
               if SOUND_DF<0,
                  ap=audioplayer(volume*xf,-afs/SOUND_DF,16,AUDIO_ID) ;
               else
                  ap=audioplayer(volume*decdc(xf,SOUND_DF),afs/SOUND_DF,16,AUDIO_ID) ;
               end
            else
               if SOUND_DF<0,
                  ap=audioplayer(volume*x(:,1:chk),-afs/SOUND_DF,16,AUDIO_ID) ;
               else
                  ap=audioplayer(volume*decdc(x(:,1:chk),SOUND_DF),afs/SOUND_DF,16,AUDIO_ID) ;
               end
            end
            play(ap) ;
            %delete(ap) ;
