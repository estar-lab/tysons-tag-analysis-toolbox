function    [POS,N,gps] = gps_posns(Obs,pos,tc,THR,kproc,excl)

%     [POS,N,gps] = gps_posns(Obs,pos,tc,[THR,kproc,excl])
...
%     N is a matrix of SV and timing quality information for each grab in POS.
...
%       N is the matrix defined above.
%		k is a vector with the observation numbers corresponding to the reliable
%		  positions in POS.
%
%		To convert POS.T into seconds since tag on, do the following:
%		gpst = gps2tag_time(POS.T, info) ;  % this is in corrected (real) time
%		gpst = gps2tag_time(POS.T-N(:,3)/24/3600, info) ; % this is in dtag clock time
%
%     markjohnson@bios.au.dk
%     www.soundtags.org
%     Last modified: 1 April 2021 - added support for refined pseudo-ranges

USEREF = 1 ;   % use refined pseudo-ranges if they are available
...
for k=1:length(Obs),  % estimate a position for each grab
   ...
   if nsv>MAX_SV,
      [mm,I] = sort(p(:,2)) ;
		p(I(1:size(p,1)-MAX_SV),2) = 1 ;
		nsv = MAX_SV ;
   end
   
   %debugging
   %p = p(p(:,2)>THR,:)
   %pp = proc_posn(p,pos,T(k,:),eph,mean(tc),THR,1) 
   %return
   %end debugging
   ...
   if pp(end-2)<SRCH_THR && pp(end-2)>1 && abs(h(k))<400,    % if positional accuracy is good update time offset
...
POS.TT = datenum(vertcat(Obs(gps.k).T)) ;
return
