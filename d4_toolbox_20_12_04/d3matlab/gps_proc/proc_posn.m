...
%     eph is the interpolated ephemeris for the period
%     tc is a time correction in seconds
%		THR is an optional snr threshold (150 is used as a default)
...
mdel = P(ksv,3) ;
[Q,R] = time_search(eph,sgps+offs1,ecef_pos,mdel) ;
...
[Q,R] = time_search(eph,sgps+tc+offs2,ecef_pos,mdel) ;
if isempty(Q), return, end
tc = tc+interpmin(offs2,R) ;
if abs(tc)>maxdev,return, end

[Q,R] = time_search(eph,sgps+tc+offs3,ecef_pos,mdel) ;
if isempty(Q), return, end
tc = tc+interpmin(offs3,R) ;
if abs(tc)>maxdev,return, end

% we now have a good idea of the correct grab time
SVP = svposns(eph,sgps+tc,Q(1:3)) ;       
if isempty(SVP),return, end
[pos,tt,m,L]=refineposn(SVP,Q(1:3),mdel) ;
SVP = svposns(eph,sgps+tc+tt,pos) ;       

% If 6 or more SVs, remove any SVs with elevation less than 10 degrees
if size(SVP,1)>5,
	[el,I] = sort(SVP(:,7)) ;
	kskip = min(sum(el)<10*pi/180,size(SVP,1)-5) ;
	kkeep = I(kskip+1:end) ;
	SVP = SVP(kkeep,:) ;
	mdel = mdel(kkeep) ;
end
	
delay = zeros(size(SVP,1),1) ;
% experimental ionospheric correction
ion_alpha = [0.1118e-07 -0.1490e-07 -0.5960e-07  0.1192e-06];           
ion_beta = [0.9830e+05 -0.1147e+06 -0.1966e+06  0.7209e+06];
ionoparams = [ion_alpha,ion_beta] ;
lalo = ecef2lla(pos) ;
delay = iono_correction(lalo(1:2)*180/pi,SVP(:,6:7),sgps+tc,ionoparams) ;
[pos,tt,m,L]=refineposn(SVP,pos,mdel,0,delay) ;

% if there are 8 or more satellites and the RMS prange error is > 30,
% try eliminating the SV with the largest prange error.
if length(L)>=8 && m>30,
   k = abs(L)<max(abs(L)) ;
   [pos1,tt1,m1]=refineposn(SVP(k,:),pos,mdel(k),0,delay(k)) ;
   if m1<m,
      pos = pos1 ;
      tt = tt1 ;
      m = m1 ;
