function    OBS = d3combineobs(recdir,prefix,fnums)
%
%    OBS = d3combineobs(recdir,prefix,fnums)
...
%		fnums is an optional argument limiting the range of files that are
%		 read.
...
for k=1:length(ff),
   ...
   if isfield(s,'obs') && ~isempty(s.obs),
      fprintf(' Reading %d grabs from %s\n',length(s.obs),ff(k).name) ;
      if isempty(OBS),
         OBS = s.obs ;
      else
