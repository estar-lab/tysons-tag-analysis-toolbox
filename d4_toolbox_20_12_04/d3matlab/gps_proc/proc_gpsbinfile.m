function    Obs = proc_gpsbinfile(fname,kproc,dtype,ns)
%
%    Obs = proc_gpsbinfile(fname,kproc,dtype,ns)
...
for k=1:3,      % try 3 times to run the gps decoder
    try
        if dtype==4,
            [P,SNR,DEL,DOP] = fast_fdoppX(B,kproc,1,1,ns) ;	% process a D4 gps bin file
        else
            FD = [-42 -25]*1e3 ;
...
T = zeros(size(kproc,1), 6);
for i = 1:size(kproc,1)
    T(i,:) = datevec(datetime(BH(kproc(i)).rtime, "ConvertFrom", "epochtime")) ;
end
