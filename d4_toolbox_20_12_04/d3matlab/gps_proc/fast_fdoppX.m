function [P,SNR,DEL,DOP] = fast_fdoppX(B,blk,findz,nb,ns)
%
% [P,SNR,DEL,DOP] = fast_fdoppX(B,blk,findz,nb,ns)
%
...
for k=1:length(blk),
   ...
   [bb,fs] = bitgrab2bb(x(ns(1):min(length(x),ns(2))),8,0,nb);
   ...
   drawnow
   if rem(k,10)==0,
      save _gpsposn_recover.mat SNR DEL DOP k
   end
end
...
% The following gives delay values in fractions of code bits
