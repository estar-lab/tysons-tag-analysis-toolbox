...
if dtype==3,
   ...
   k = find(all(xb==-1,2)) ;
   if(k>0), fprintf(' fill %d\n',length(k)); end
   xb(k,:) = NaN ;                  % replace fill words with NaN
end
...
for k=1:length(uchans),
   ...
   if ~isempty(kk),
      if any(kk>size(xb,2)),
         fprintf('Mismatch between channel descriptors and swv channels - cannot read data\n') ;
         X = [] ;
         return
      end
      x{k} = reshape(xb(:,kk)',[],1) ;
