function    [SL,f] = trimmedSL(x,fs,nfft,freqr,ptrim)
%
%    [SL,f] = trimmedSL(x,fs,nfft,freqr,ptrim)
%     Compute the SL of a sound vector.
...
if nargin==5 && ~isempty(ptrim),
   ...
   P = sum(ff(1:nfft/2,kp),2)/length(kp) ;      
else
   P = sum(ff(1:nfft/2,:),2)/size(x,2) ;
end
