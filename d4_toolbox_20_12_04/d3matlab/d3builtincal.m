...
%              26/03/24 fixed bug at line 58 affecting calibration strings with negative numbers
...
for k=1:length(cfg.CAL),
   ...
   cc = cfg.CAL{k}.CAL ;
	cc(cc=='_') = '-' ;     % fixed bug here 26/03/24. This line was after following line.
	cc = str2num(cc) ;
	psel(km(1)).cal = cc ;
end
         ...
         for ko=1:length(cfg.OFFSET),
            offs = cfg.OFFSET{ko} ;
		      if strcmp(offs.SENS,'PRES'),
      			psel(k).poffs = str2num(offs.OFFSET) ;
		      end
		      if strcmp(offs.SENS,'TEMP'),
      			psel(k).toffs = str2num(offs.OFFSET) ;
            end
         end
      end
