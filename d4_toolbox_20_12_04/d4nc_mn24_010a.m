...
recdir = ['C:\Users\tyson\Documents\ESTAR\GPS\', depid];
...
% This is good enough to go downstairs to do GPS stuff
% -Tyson
...
info = make_info(depid,'D4');%%%%%,'dz'
info.project_name = 'Maui - 2024';

info.project_datetime_start = '10January24 08:30:00'; %local time
info.dephist_deploy_locality = 'Maui';
info.dephist_deploy_method = 'suction cup';
%info.dephist_animal_id = 'suction cup';
...
time_tag_on_char = info.project_datetime_start;
...
plot3(mag(idx_cal,1),mag(idx_cal,2),mag(idx_cal,3),'.')
hold on
plot3(1,0,0,'rx')
plot3(0,0,1,'rx')
hold off
...
[mag, idx_cal] = mag_calib(M.data, A0.data, -P0.data);
M.data = mag;
...
P = apply_cal(P0,CAL.PRESS,T) ;		% apply cal to the pressure. For Tag405calibration constants [2361.13000000000,-98] 
...
%% Save the data and update/save the CAL file
...
lat = 42.2959; % Location of North Campus
long = -83.7103; 
