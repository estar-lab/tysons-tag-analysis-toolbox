function [roll, pitch, yaw, A_g, A_spec, bool_singular] =...
  calc_rpy_magnetometer_adv(A, M, sample_freq, wd_static, wd_naive)
% A testing advanced implementation of the "magnetometer method" proposed 
% in [1]. Rather than assuming all dynamic rotations are happening in the 
% "pitching" plane, this implementation does not make that assumption and
% use dynamic rotations identified from magnetometer to infer dynamic
% rotations in accelerometer, to decouple linear acceleration generated by
% the animal from the acceleration measurements result from gravity.
%
% See "calc_rpy_magnetometer.m" for the "magnetometer method"
% implementation. The "magnetometer method" uses 
...
parfor t = 1:length(A)
  ...
  if M_filt(t, 3)^2 + M_filt(t, 1)^2 >= M_filt(t, 2)^2*singular_const
    % Not singular.
    rot_mat = vrrotvec2mat(vrrotvec(M(t, :), M_filt(t, :)));
    A_spec(t, :) = A(t, :) - A_filt(t, :)*rot_mat ;
  else
    % Singular.
