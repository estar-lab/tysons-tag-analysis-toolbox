function [A_spec, A_g, roll, pitch, yaw, rot_mat] = madgwick_filt(A, W, M, sample_period)
...
% 'W'       n-by-3 Gyroscope reading, needs to be calibrated with no bias.
%           [rad] (has to be)
% 'M'       n-by-3 Magnetometer reading, needs to be calibrated with no bias.
%           [unit does not matter, to be normalized]
% 'sample_period'   Sampling period of sensors.
%
% Output:
%
...
AHRS = MadgwickAHRS('SamplePeriod', sample_period, 'Quaternion',...
    quaternion_initial, 'Beta', Beta);
quaternion = zeros(length(W), 4);
% Main algorithm loop.
for t = 1:length(W)
    AHRS.Update(W(t,:), A(t,:), M(t,:));	% gyroscope units must be radians
