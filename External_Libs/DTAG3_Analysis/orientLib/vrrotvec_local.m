function r = vrrotvec_local(a, b, options)
...
an = sl3dnormalize_local(a, epsilon);
bn = sl3dnormalize_local(b, epsilon);
...
ax = sl3dnormalize_local(cross(an, bn), epsilon);
...
if ~any(ax)
    absa = abs(an);
    [dmvar01, mind] = min(absa);
    c = zeros(1,3);
    c(mind) = 1;
    ax = sl3dnormalize_local(cross(an, c), epsilon);
end
