...
if ~isfield(CAL.ACC,'XC')
   CAL.ACC.XC = zeros(3) ;
end
if ~isfield(CAL.ACC,'PC')
   CAL.ACC.PC.POLY = zeros(3,1) ;
end
...
% exclude acceleration measurements with high jerk or that are too shallow
jj = [norm2(diff(A));0] ;
thr = prctile(jj,50) ;
kg = find((jj(ks)<thr) & (p(ks)>mindepth) & (any(isnan(A(ks,:)),2)==0)) ;
kk = ks(kg) ;
A = A(kk,:) ;
