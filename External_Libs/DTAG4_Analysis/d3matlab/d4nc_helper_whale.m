addpath(genpath('D:\humpbackWhale\d4_toolbox_20_12_04'))
...
% recdir=['C:\Users\jtgab\Box Sync\Grad Research\Dolphin Analysis\data\', depid];
% recdir = ['G:\proc_tag\Stellewagen\', depid]; % Work Win10
% recdir = ['D:\humpbackWhale\data_mn18_172b\', depid]; % Home Win10
recdir = ['D:\humpbackWhale\data_mn18_172a\']; % Home Win10
...
% or
X = d3readswv(recdir,depid,5) ;		% decimates each channel by a factor of dec
...
% to compute the field strength from the latitude and longitude values.
% If lat is negative, set the site option to "S", if long is negative, set the
% site option to "W".
% Ensure start and end dates are also entered correctly.
% Site reports in nT, convert these to uT.
field = 51.3657; % Units of uT
...
% ONLY PERFORM THIS NEXT STEP IF ABSOLUTELY NECESSARY, CAN BREAK A GOOD P CAL
...
% If desired, attempt autocalibration of magnetometer data
% Plot MM or norm2(MM) to make sure it looks good. If it does, save the
% improved calibration:
% [MM,mc] = auto_cal_mag(M,CAL.MAG,field,T);
% CAL.MAG = mc;

% Apply the calibration and map to get the final tag-frame magnetometer data:
M = do_cal(M,CAL.MAG);
...
% [tc,rerr] = gps_timesearch(OBS,[lat,long],-6*3600+[-30 30],THR);
