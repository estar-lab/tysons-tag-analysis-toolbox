...
if (nargin < 3)
    ...
    targetDirectory = convertStringsToChars(targetDirectory);
end
...
if any(str == '*')
    list = h.helper.InternalJObject.listNames(str);
...
while iListing < length(listing)
    ...
    if fileObject.exists && ~fileObject.delete
        error(message("MATLAB:io:ftp:ftp:CannotOverwrite", localName))
    end
    ...
    if ~canOpen
        error(message("MATLAB:io:ftp:ftp:CannotCreate", localName));
    end

    % Download the file.
    fos = java.io.FileOutputStream(fileObject);
    try
        status = h.helper.InternalJObject.retrieveFile(name, fos);
        if ~status
            % possibly input was a directory or file was missing
            org.apache.commons.io.IOUtils.closeQuietly(fos);
            [location,listing] = replyCodeHelper(h, fileObject, name, ...
                localName, location, listing);
            continue;
        else
            fos.flush();
        end
    catch ME
        if any(ME.identifier == ["MATLAB:io:ftp:ftp:FileUnavailable", ...
                "MATLAB:io:ftp:ftp:FTPError"])
            throw(ME);
        end
        % close the FTP connection, and reconnect
        org.apache.commons.io.IOUtils.closeQuietly(fos);
        close(h);
        connect(h.helper);
        replyCodeHelper(h, fileObject, name, localName, location, listing);
    end
    % if exception was not thrown, close the connection
    org.apache.commons.io.IOUtils.closeQuietly(fos);
    fschange(fileparts(localName));
    [location,listing] = replyCodeHelper(h, fileObject, name, localName, ...
        location, listing);
end
end

function [location, listing] = replyCodeHelper(h, fileObject, name, ...
    localName, location, listing)
...
switch replyCode
    ...
    case 550
        ...
        if isDir
            ...
            error(message("MATLAB:io:ftp:ftp:FileUnavailable", name));
        end
    otherwise
