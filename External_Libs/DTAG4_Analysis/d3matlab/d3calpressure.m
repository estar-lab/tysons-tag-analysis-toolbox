function    [p,CAL,fs,t,fst] = d3calpressure(X,CAL,test)

%    [p,CAL,fs,t,fst] = d3calpressure(X,CAL,[test])
...
if fst/df<fs,
   t = interp2length(t,fst/df,fs,length(p)) ;
end
...
t = interp2length(t,fst,fsin,length(X.x{kp})) ;
%p = apply_cal(X.x{kp},CAL.PRESS,[],t) ;
p = apply_cal(X.x{kp},CAL.PRESS,t) ;

fs = fsin ;
if isfield(CAL.PRESS,'EXPCORR'),
   c = exp(-(1:length(p))'/(CAL.PRESS.EXPCORR(2)*fs));  
   p = p+CAL.PRESS.EXPCORR(1)*c;
end
return
